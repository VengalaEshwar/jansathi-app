...
const SchemeCard = memo(({
  scheme, showScore = false, onPress,
}: {
      ...
      >
        <View style={[{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }, S.mb8]}>
          <View style={{ flex: 1, marginRight: 8 }}>
            <Text className="font-bold text-sm text-[#0F172A] dark:text-white" numberOfLines={2}>
              {scheme.name}
            </Text>
        ...
        <Text className="text-[#64748B] dark:text-[#94A3B8] text-xs leading-4" numberOfLines={3} style={S.mb8}>
          {scheme.description}
        </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={{ flexDirection: "row", gap: 6 }}>
            {scheme.tags.slice(0, 4).map((tag) => (
              <View key={tag} className="bg-[#F1F5F9] dark:bg-[#334155]"
                style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 99 }}>
                <Text className="text-[#64748B] dark:text-[#94A3B8] text-xs">{tag}</Text>
              </View>
            ))}
            {scheme.tags.length > 4 && (
              <View className="bg-[#F1F5F9] dark:bg-[#334155]"
                style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 99 }}>
...
export default function SchemeFinder() {
  ...
  return (
    <ScrollView className="flex-1 bg-[#F8FAFC] dark:bg-[#0F172A]">
      ...
      {/* ── Header area (scrollable, not fixed) ── */}
      <Animated.View
        style={[headerAnim, { paddingHorizontal: sidePad, paddingTop: 16 }]}
        className="bg-[#F8FAFC] dark:bg-[#0F172A]"
      >
        {/* Web spacer — fixes NativeWind v4 first-child gap on web */}
        ...
        {/* Web spacer — gap between HeroSection bottom and mode toggle below */}
        {Platform.OS === "web" && <View style={{ height: 8 }} />}

        {/* Mode toggle — active pill fills parent height, no alignItems on parent */}
        <View
          className="bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155]"
          style={{
            ...
            // NO alignItems — children stretch to fill full height
          }}
        >
          {([
      ...
      {mode === "browse" && (
        <Animated.View style={[bodyAnim, { flex: 1 }]}>
          ...
          {/* Search + filter bar */}
          <View style={{
            flexDirection: "row", paddingHorizontal: sidePad,
          ...
          <Pressable
            onPress={() => { playClick("soft"); setShowCategories(!showCategories); }}
            style={{
              flexDirection: "row", justifyContent: "space-between",
              alignItems: "center", paddingHorizontal: sidePad, paddingVertical: 8,
            }}
          >
                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <AnimatedPressable
                      key={cat}
                      onPress={() => {
                        const next = cat !== "All" ? (selectedCategory === cat ? "" : cat) : "";
                        setSelectedCategory(next);
                        fetchSchemes(1, { category: next });
                      }}
                      soundType="soft"
                      style={{
          ...
          {isLoading ? (
            ...
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 32 }}>
              ...
              <Text className="font-bold text-base text-[#0F172A] dark:text-white text-center" style={S.mb8}>
                {ts.exploreTitle}
              </Text>
              <Text className="text-[#64748B] dark:text-[#94A3B8] text-center text-sm" style={S.mb20}>
                {ts.exploreDesc}
              </Text>
              <AnimatedPressable
                onPress={() => fetchSchemes(1)}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                isWide ? null : <SchemeCard scheme={item} onPress={handleSchemePress} />
              )}
      ...
      {mode === "eligible" && (
        <Animated.View style={[bodyAnim, { flex: 1 }]}>
          {Platform.OS === "web" && <View style={{ height: 8 }} />}
          <ScrollView
            contentContainerStyle={{ paddingHorizontal: sidePad, paddingTop: 8, paddingBottom: 100 }}
            ...
            {!eligibilityDone ? (
              <View>
                ...
                <PickerRow label={ts.occupation} value={form.occupation} onChange={(v) => setForm({ ...form, occupation: v })}
                  options={[
                    { label: ts.student, value: "student" }, { label: ts.farmer, value: "farmer" },
              ...
              <View>
                {/* Result banner */}
                <View
                  className="bg-green-100 dark:bg-green-900/20 border border-green-300 dark:border-green-800"
                ...
                {eligibleSchemes.length === 0 ? (
                  ...
                  eligibleSchemes.map((s) => (
                    <SchemeCard key={s._id} scheme={s} showScore onPress={handleSchemePress} />
                  ))
                )}

                <AnimatedPressable
                  onPress={() => { setEligibilityDone(false); setEligibleSchemes([]); }}
                  soundType="soft"
                  style={{
                    marginTop: 12, paddingVertical: 14, borderRadius: 16,
                    borderWidth: 2, alignItems: "center", borderColor: "#8B5CF6",
                  }}
                >
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }}>
          <View className="bg-white dark:bg-[#1E293B]"
                ...
                <View style={{ flexDirection: "row", gap: 8 }}>
                  {INDIAN_STATES.map((s) => {
                    ...
                    return (
                      <AnimatedPressable
                        key={s}
                        onPress={() => setSelectedState(isSelected ? "" : s)}
                        soundType="soft"
                        style={{
                          paddingHorizontal: 14, paddingVertical: 9,
                          borderRadius: 99, borderWidth: 1,
            ...
            <View style={[{ flexDirection: "row", marginTop: 14 }, S.gap10]}>
              <AnimatedPressable
                onPress={() => { setSelectedCategory(""); setSelectedState(""); }}
                soundType="soft"
              ...
              <AnimatedPressable
                onPress={() => { setShowFilter(false); fetchSchemes(1); }}
                soundType="mechanical"
                style={{
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }}>
          <View className="bg-white dark:bg-[#1E293B]"
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <Pressable
                  onPress={() => { playClick("soft"); setForm({ ...form, state: item }); setShowStatePicker(false); }}
      ...
      <Modal visible={!!selectedScheme} animationType="slide" transparent>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }}>
          <View className="bg-white dark:bg-[#1E293B]"
            style={{ borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: "85%" }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              <View style={[{ flexDirection: "row", flexWrap: "wrap" }, S.gap8, S.mb14]}>
                ...
                {selectedScheme?.state.slice(0, 2).map((s) => (
                  <View key={s} className="bg-[#F1F5F9] dark:bg-[#334155]"
              ...
              <Text className="text-[#64748B] dark:text-[#94A3B8] text-sm leading-5" style={S.mb14}>
